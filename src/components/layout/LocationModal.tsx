...
export function LocationModal() {
  const { address, lat, lng, deliveryType, modalOpen, closeModal, setAddress, setDeliveryType } =
    useLocationStore();
  ...
  function handleAddressCoordinates(newLat: number, newLng: number) {
    // Mapbox emite el texto y las coordenadas en callbacks consecutivos. Leemos el
    // store para no recuperar una dirección anterior desde el closure de React.
    setAddress(useLocationStore.getState().address, newLat, newLng);
  }

  return (
    <div className="fixed inset-0 z-[100] flex items-start justify-center bg-black/50 pt-16 backdrop-blur-sm md:pt-24">
        ...
        <div className="space-y-5 p-5">
          ...
          <div className="flex rounded-lg bg-[#f5f5f5] p-1">
            <button
              ...
              className={`flex-1 rounded-md py-2.5 text-xs font-bold tracking-wider uppercase transition-all ${
          ...
          <div>
            <div className="mapbox-autocomplete">
              <MapboxAutocomplete
                ...
                onCoordinates={handleAddressCoordinates}
          ...
          <button
            ...
            className="text-on-surface hover:border-primary hover:text-primary flex w-full items-center justify-center gap-2 rounded-xl border-2 border-[#e5e5e5] py-3 text-xs font-bold tracking-wider uppercase transition-all active:scale-[0.98]"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="1em"
