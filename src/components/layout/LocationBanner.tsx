...
export function LocationBanner() {
  ...
  useEffect(() => {
    // No mostrar si ya tenemos ubicación, fue dismisseado, o el browser no lo soporta
  ...
  async function handleAllow() {
    ...
    if (loc) {
      ...
      dismiss(); // No volver a preguntar si denegó
    }
  }
      ...
      <div className="bg-surface-container-lowest rounded-3xl shadow-[0_8px_40px_rgba(89,65,61,0.18)] border border-outline-variant/20 overflow-hidden">
        <div className="p-4 flex gap-3">
          ...
          <button
            ...
            className="w-7 h-7 flex items-center justify-center rounded-full text-on-surface-variant hover:bg-surface-container-high transition shrink-0"
          >
        ...
        <div className="px-4 pb-4 flex gap-2">
          <button
            onClick={handleDismiss}
            className="flex-1 py-2.5 rounded-xl text-[11px] font-bold uppercase tracking-wider text-on-surface-variant bg-surface-container-high transition active:scale-95"
          >
            Ahora no
          </button>
          <button
            onClick={handleAllow}
            disabled={loading}
            className="flex-1 py-2.5 rounded-xl text-[11px] font-bold uppercase tracking-wider bg-primary text-on-primary shadow-lg shadow-primary/25 transition active:scale-95 disabled:opacity-60 flex items-center justify-center gap-1"
          >
            {loading ? (
