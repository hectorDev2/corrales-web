...
export function KfcProductCarousel({ title, products, href }: KfcProductCarouselProps) {
  ...
  return (
    <section className="relative py-12">
      ...
      <div
        ref={carouselRef}
        className="flex overflow-x-auto gap-8 pb-4 hide-scrollbar snap-x snap-mandatory"
      >
        {products.map((product) => {
          ...
          return (
            <div
              key={product.id}
              className="min-w-[280px] md:min-w-[300px] flex-1 snap-start bg-white border border-[#F2F2F2] relative group"
            ...
            >
              {/* Image */}
              <a href={`/producto/${product.id}`} className="block aspect-square w-full p-4">
                <img
              ...
              <div className="p-5 flex flex-col h-[200px]">
                <h3 className="text-[18px] font-bold mb-2 leading-tight">
                  <a href={`/producto/${product.id}`} className="hover:text-primary transition-colors">
                    {product.name}
                  </a>
                </h3>
                <p className="text-secondary text-[14px] leading-relaxed line-clamp-3 mb-auto">
                  {product.description}
                </p>
              ...
              <QuickAddButton
                product={product}
                className="absolute bottom-4 right-4 w-10 h-10 bg-primary text-white flex items-center justify-center transition-transform active:scale-90 hover:brightness-110"
              >
