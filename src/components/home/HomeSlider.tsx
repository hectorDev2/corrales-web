      ...
      <div ref={emblaRef} className="w-full overflow-hidden">
        <div className="flex w-full touch-pan-y items-start">
          {slides.map((slide) => (
            <div
              key={slide.id}
              className="relative min-w-0 flex-[0_0_100%] overflow-hidden bg-black"
            >
              {/* Desktop: 320px, Mobile: 180px */}
              <div className="h-[180px] md:h-[320px] relative">
                {slide.type === "image" && slide.image_url ? (
                  <>
                    ...
                    {slide.cta_label && slide.cta_href && (
                      <Link
                        href={slide.cta_href}
                        className="absolute bottom-4 left-4 flex items-center gap-2 rounded-full bg-white px-4 py-2.5 text-sm font-bold text-primary shadow-lg transition-transform hover:scale-105 active:scale-95"
                      >
                        <span
                  ...
                  <>
                    <div className={`absolute inset-0 bg-gradient-to-br ${slide.bg_gradient ?? "from-[#e4002b] via-[#b30022] to-[#800018]"}`} />
                    ...
                    {/* Decorative circles */}
                    <div className="absolute -top-16 -right-16 h-72 w-72 rounded-full opacity-10" style={{ background: slide.accent_color ?? "#e4002b" }} />
                    <div className="absolute -right-8 bottom-0 h-48 w-48 rounded-full opacity-[0.07]" style={{ background: slide.accent_color ?? "#e4002b" }} />
                      ...
                      {slide.eyebrow && (
                        <div className="mb-2 flex items-center gap-2">
                          {slide.icon && (
                            <span
                              className="material-symbols-outlined text-base"
                              style={{ color: slide.accent_color ?? "#e4002b", fontVariationSettings: "'FILL' 1, 'wght' 400, 'GRAD' 0, 'opsz' 24" }}
                              aria-hidden="true"
                            >
                              {slide.icon}
                            </span>
                          )}
                          <span className="text-xs font-bold uppercase tracking-widest" style={{ color: slide.accent_color ?? "#e4002b" }}>
                            {slide.eyebrow}
                          </span>
                        </div>
                      )}
